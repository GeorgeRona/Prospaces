...
import { MarketingLogin } from './components/marketing-app/MarketingLogin';
import { MarketingShell } from './components/marketing-app/MarketingShell';
...
function MarketingApp() {
  ...
  if (isMobile) {
    return <DesktopOnlyAccess spaceName="Marketing Space" accentColorClass="bg-rose-600" />;
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="text-slate-500">Loading Marketing Space...</p>
        </div>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <ErrorBoundary>
        <Toaster />
        <MarketingLogin onLogin={handleLogin} />
      </ErrorBoundary>
    );
  }
  ...
  return (
    <ErrorBoundary>
      <ThemeProvider userId={user.id}>
        <Toaster />
        <MarketingShell user={user} accessToken={accessToken} onLogout={handleLogout} />
      </ThemeProvider>
    </ErrorBoundary>
  );
}
